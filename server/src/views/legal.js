...
function privacyHtml() {
  const dataTable = table(
    ...
    [
      [
        '设备与版本信息',
        'Android ID；设备名称、厂商、型号、系统与内核版本、CPU 架构、内存与存储容量、'
          + '屏幕分辨率、安全补丁级别、构建编号与指纹、基带版本、开机时长、电池健康与电量、应用版本号',
        '<strong>是</strong>，每个版本首次启动时上报一次',
        '统计安装量与版本分布，排查特定机型或系统版本上的问题',
      ],
      [
        '录制脚本备份',
        '仅你创建的录制脚本（即「导出」功能涉及的内容）：脚本名称与其中的点击、滑动、等待动作。'
          + '滑动速度、关键词、统计数据等本机设置<strong>不再上传</strong>',
      ...
      [
        '网络出口地址',
        '本机的公网出口 IP 与其归属地（省市级）',
        '<strong>是</strong>，随上报一起提交',
        '统计用户地域分布',
      ],
  ...
  const body = [
    ...
    section('三、数据的存储与保留', [
      '上传的数据存储在开发者的服务器上，用于统计、备份与反馈沟通。',
      '本应用不接入任何第三方广告或数据分析 SDK。文字识别使用的 Google ML Kit 模型随安装包内置，识别过程完全在本机完成，不联网。',
      '本应用会向以下第三方站点发起请求：检查更新时访问 GitHub 获取版本信息；'
        + '探测网络出口地址时访问 ip138（2026.ip138.com）。这两个请求都不携带 Android ID 等设备标识，'
        + '但对方站点会像任何网站一样看到你的 IP 地址。',
    ]),
    section('四、你的权利', [
