...
/**
 * 通过 Shizuku UserService 桥接执行 priv-kit 原生启动器。
 *
 * 逻辑与 priv-kit 官方示例一致：先绑定一个运行在 Shizuku 进程中的
 * [PrivilegeShizukuStartService]，再把 priv-kit 的启动命令交给它执行，
 * 最终以 shell/root 身份拉起特权服务端并完成 Binder 握手。
 */
internal class PrivilegeShizukuExternalStarter(
    ...
    private suspend fun bindOrGetService(): IPrivilegeShizukuStartService {
        service?.takeIf { current ->
            runCatching { current.asBinder().pingBinder() }.getOrDefault(false)
        }?.let { return it }
    ...
    private fun removeUserService(connection: ServiceConnection) {
        ...
        // remove=false 只清除 Shizuku 客户端侧连接缓存；remove=true 销毁远端记录。
        runCatching {
    ...
    companion object {
        ...
        private const val SHIZUKU_START_TAG = "autoslide-priv-kit-start"
        private const val SHIZUKU_START_PROCESS_SUFFIX = "autoslide-priv-kit"
        // 每次修改 AIDL 契约或服务实现时递增。
        private const val SHIZUKU_START_SERVICE_VERSION = 1
    }
}
